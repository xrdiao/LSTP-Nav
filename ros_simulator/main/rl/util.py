...
class AttentionAgent(nn.Module):
    def __init__(self, env):
        ...
        kernel_size = 5
        stride = 2
        # input_size = (LASER_NUM - kernel_size) // stride + 1
        input_size = LASER_NUM
        self.pool = nn.MaxPool1d(kernel_size=kernel_size, stride=stride)

        self.lstm_actor = nn.LSTM(input_size=input_size, hidden_size=self.hidden_dim, num_layers=3, batch_first=True)
