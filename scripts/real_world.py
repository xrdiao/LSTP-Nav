...
class RealWorld:
    ...
    def main(self):
        ...
        agent = LstmAgent().to(device)
        agent.load_state_dict(torch.load(MODEL_DIR / "Agent_Lstm_base.pth", map_location=device))

        # agent=AttentionAgent()
        # agent.load_state_dict(torch.load(MODEL_DIR / "AttentionAgent_circle.pth", map_location=device))
        ...
        while True:
            if self.last_odom == None:
                ...
                state = torch.Tensor([relative_pos+cur_vel]).to(device)
                # print(self.name, state)

                laser_datas = []
                laser_data = [i for i in self.laser_buffer]
                laser_datas.append(torch.clip(torch.Tensor(laser_data),0.02,4).to(device))
                laser_datas = torch.stack(laser_datas).to(device)
                # print(laser_data)
